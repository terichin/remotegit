...
public class CsvDao {
    ...
    // 社員情報をMemberBeanで扱うため戻り値の型をMemberBeanに変更
    public static MemberBean search(int key, String query) {
        ...
        while (true) {
            ...
            //その社員情報をMemberBeanで返す
            if (record[key - 1].indexOf(query) != -1) {
            	return new MemberBean(record);
            }
            index++;
        }
    }

    //社員情報をMemberBeanで扱うため、引数をMemberBean型に変更
